...
def get_detections(segment_key, flag):
    ...
    d_p_boxes, scores_persons, class_id_humans, keypoints_persons = get_boxes_det(persons_d, img_H, img_W)
    d_o_boxes, scores_objects, class_id_objects, keypoints_objects = get_boxes_det(objects_d, img_H, img_W)
    if len(d_p_boxes) > select_threshold:
        d_p_boxes, scores_persons, class_id_humans, keypoints_persons = d_p_boxes[0:select_threshold], scores_persons[
                                                                                                       0:select_threshold], class_id_humans[
                                                                                                                            0:select_threshold], keypoints_persons[
                                                                                                                                                 0:select_threshold]
    if len(d_o_boxes) > select_threshold - 1:
        d_o_boxes, scores_objects, class_id_objects, keypoints_objects = d_o_boxes[
                                                                         0:select_threshold - 1], scores_objects[
                                                                                                  0:select_threshold - 1], class_id_objects[
                                                                                                                           0:select_threshold - 1], keypoints_objects[
                                                                                                                                                    0:select_threshold - 1]
    scores_objects.insert(0, 1)
    return d_p_boxes, d_o_boxes, scores_persons, scores_objects, class_id_humans, class_id_objects, annotation, shape, keypoints_persons, keypoints_objects


def get_compact_detections(segment_key, flag):
    d_p_boxes, d_o_boxes, scores_persons, scores_objects, class_id_humans, class_id_objects, annotation, shape, keypoints_persons, keypoints_objects = get_detections(
        segment_key, flag)
    img_W = shape[0]
    img_H = shape[1]
    ...
    keypoints_persons_np = np.array(keypoints_persons, np.float32)
    keypoints_objects_np = np.array(keypoints_objects, np.float32)
    keypoints_persons_np = keypoints_persons_np / np.array([img_W, img_H])
    if no_object_dets != 0:
        keypoints_objects_np = keypoints_objects_np / np.array([img_W, img_H])

    return {'person_bbx': persons_np, 'objects_bbx': objects_np,
            'person_bbx_score': scores_persons, 'objects_bbx_score': scores_objects,
            'class_id_objects': class_id_objects, 'keypoints_persons': keypoints_persons_np,
            'keypoints_objects': keypoints_objects_np}
...
def get_attention_maps(segment_key, flag):
    ...
    keypoints_persons, keypoints_objects = compact_detections['keypoints_persons'], compact_detections[
        'keypoints_objects']
    keypoints = []
    union_box = []
    ...
    for dd_i in range(no_person_dets):
        for do_i in range(len(objects_np)):
            ...
            if len(keypoints_objects) == 0:
                obj_keypoints = []
                for num in range(num_keypoints):
                    temp = []
                    temp.append(0)
                    temp.append(0)
                    obj_keypoints.append(temp)

                keypoints.append(np.concatenate((keypoints_persons, obj_keypoints), axis=0))


            else:
                keypoints.append(np.concatenate((keypoints_persons, keypoints_objects), axis=0))
                # if len(keypoints_objects)>2:
                #     x = [keypoints_objects[0], keypoints_objects[1]]
                # else:
                #     x = keypoints_objects
                # temp = np.concatenate((keypoints_persons, x), axis=0)
                # keypoints.append(temp)

    return np.concatenate(union_box), np.array(keypoints)
...
def get_boxes_det(dets, img_H, img_W):
    ...
    return boxes, scores, class_no, keypoints
