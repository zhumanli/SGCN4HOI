...
def get_pool_loc(ims, image_id, flag_, size=(7, 7), spatial_scale=1, batch_size=1):
    ...
    for batch in range(batch_size):
        ...
        # keypoints_persons = a['keypoints_persons']
        # keypoints_objects = a['keypoints_objects']
        W, H, C = ims[batch].size()[1], ims[batch].size()[2], ims[batch].size()[0]
        # keypoints_persons = keypoints_persons * [W, H]
        # keypoints_objects = keypoints_objects * [W, H]
        # keypoints_out.append(torch.tensor(np.concatenate((keypoints_persons, keypoints_objects), axis=0)).cuda().float())
        union_box, keypoints = helpers_pre.get_attention_maps(this_image, flag)
        keypoints = keypoints * [W, H]
        keypoints_out.append(torch.tensor(keypoints).cuda().float())
    ...
    # keypoints_persons_tensor = []
    # for x in keypoints_persons:
    # 	keypoints_persons_tensor.append(torch.from_numpy(x))
    # keypoints_objects_tensor = []
    # for x in keypoints_objects:
    # 	keypoints_objects_tensor.append(torch.from_numpy(x))
